...
export class AIDecisionEngine {
  ...
  constructor(learningData: AILearningData, preferences: UserPreferences) {
  ...
  async generateEnhancedRecommendation(
    ...
    const aiInsights = await this.generateAIInsights(date, courses, schedule, baseRecommendation);
    const confidence = this.calculateConfidence(baseRecommendation, date);
  ...
  private async generateAIInsights(
    ...
    const historicalPattern = this.analyzeHistoricalPattern(dayOfWeek);
  ...
  private calculateConfidence(recommendation: DayRecommendation, date: string): number {
  ...
  private analyzeHistoricalPattern(dayOfWeek: string) {
    const dayData = this.learningData.historicalAttendance.filter(
